...
import Nash.Json.Decode qualified as D
import Nash.Json.Encode qualified as E
import Nash.Json.String qualified as Json
import Nash.Parse.Primitives (Col, Row)
import Nash.Parse.Primitives qualified as P
import Nash.Reporting.Suggest qualified as Suggest
import Nash.Version qualified as V
...
isKernel (Name author _) =
    author == nash || author == elm_explorations
...
suggestions =
    let
        random = toName nash "random"
        time = toName nash "time"
        file = toName nash "file"
     in
        Map.fromList
            [ "Browser" ==> browser
            , "File" ==> file
...
authorDistance given possibility =
    if possibility == nash || possibility == elm_explorations
        then 0

...
import Nash.String qualified as ES
        ...
        if isDoubleQuote pos end
            then
                let
                    !pos1 = plusPtr pos 1
                 in
                    case if isDoubleQuote pos1 end
                        then
                            ...
                            singleString pos1 end row (col + 1) pos1 mempty of
    ...
    if pos >= end || P.unsafeIndex pos /= 0x7B {- { -}
        then
            ...
            let
             ...
             in
                if newPos >= end || P.unsafeIndex newPos /= 0x7D {- } -}
                    then

...
import Nash.Parse.Primitives (Col, Parser, Row, unsafeIndex)
                        ...
                        !upperName =
                            if upperStart == pos
                                then
                                    ...
                                    let !home = Name.fromPtr pos (plusPtr upperStart (-1))
                                     in Qualified home name
                     in
                     ...
                     in
                        if alphaStart == pos
                            then
                                ...
                                let !home = Name.fromPtr pos (plusPtr alphaStart (-1))
                                 in cok (Src.VarQual varType home name) newState
...
chompUpper pos end col =
    let !width = getUpperWidth pos end
     in if width == 0
            then
                (# pos, col #)
            else
...
chompLower pos end col =
    let !width = getLowerWidth pos end
     in if width == 0
            then
                (# pos, col #)
            else

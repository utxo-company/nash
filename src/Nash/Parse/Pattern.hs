...
import Nash.Parse.Primitives (Parser, addEnd, addLocation, getPosition, inContext, oneOf, oneOfWithFallback, word1, word2)
        ...
        if pos == end || P.unsafeIndex pos /= 0x5F {- _ -}
            then
                ...
                let
                    ...
                    !newCol = col + 1
                 in
                    if Var.getInnerWidth newPos end > 0
                        then

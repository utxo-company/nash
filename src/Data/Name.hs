...
instance Chars.IsString (Utf8.Utf8 NASH_NAME) where
    fromString = Utf8.fromChars

instance Binary.Binary (Utf8.Utf8 NASH_NAME) where
    ...
    if index <= 0
        then
            ...
            let
             ...
             in
                if isTrue# (leWord# 0x30## end#) && isTrue# (leWord# end# 0x39##)
                    then
                        runST
                            ( do
